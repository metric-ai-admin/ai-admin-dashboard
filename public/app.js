...
async function amApplyOutlookRules() {
  ...
  if (!btn) return;
  if (!confirm("This will create/update 15 Outlook message rules in Lyndsay's mailbox (Fire Claim, Lyndsay Review, Financial, Archive, MPM Team, Personal — incl. auto-forwards to Claudia/Accounting). Continue?")) return;
  const label = btn.textContent;
